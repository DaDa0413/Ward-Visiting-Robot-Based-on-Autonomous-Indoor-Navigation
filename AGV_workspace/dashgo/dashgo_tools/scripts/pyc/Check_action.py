...
class CheckServer():
	...
	min_turn_speed=0.1
        ...
        while not rospy.is_shutdown():
			...
			rospy.loginfo("current_v ji suan error")
		    #rospy.loginfo("current_v = "+ str(current_v)+" self.speed="+str(self.speed))
		    if math.fabs(current_v) >= math.fabs(self.speed):
			current_v= self.speed
		    elif math.fabs(current_v) < math.fabs(min_turn_speed) :
